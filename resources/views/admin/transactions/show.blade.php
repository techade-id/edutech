                ...
                <div class="sm:flex flex-row justify-between">
                    ...
                    <div class="flex flex-col gap-y-6">
                        <div>
                            ...
                            <span class="text-white text-xs -ml-1 inline-block px-3 py-2 rounded-full bg-green-500 font-bold">ACTIVE</span>
                            @else
                            <span class="text-white text-xs -ml-1 inline-block px-3 py-2 rounded-full bg-orange-500 font-bold">PENDING</span>
                            @endif
                        </div>
                        <div class="">
                            <p class="text-slate-500 text-sm">Student Name</p>
                            <h3 class="text-indigo-950 font-bold">{{ $subscribeTransaction->user->name }}</h3>
                        </div>
                        <div>
                            <p class="text-slate-500 text-sm">Total Amount</p>
                            <h3 class="text-indigo-950 font-bold">Rp {{ number_format($subscribeTransaction->total_amount, 0 , ',' , '.') }}</h3>
                        </div>
                        <div>
                            <p class="text-slate-500 text-sm">Checkout Date</p>
                            <h3 class="text-indigo-950 font-bold">{{ $subscribeTransaction->created_at->format('d M Y - G:i') }}</h3>
                        </div>
                        <div>
                            <p class="text-slate-500 text-sm">Subscription Start At</p>
                            @if ($subscribeTransaction->is_paid)
                            <h3 class="text-indigo-950 font-bold">{{ \Carbon\Carbon::parse($subscribeTransaction->subscribe_transactions)->format('d M Y - G:i') }}</h3>
                            @else
                            <h3 class="text-red-500 font-bold">Not started yet</h3>
                            @endif
                        </div>
                    </div>
                    <div class="mt-6 sm:mt-0">
                        <p class="text-slate-500 text-sm">Bukti Pembayaran:</p>
                        <img class="w-[350px] lg:w-[280px]" src="{{ Storage::url($subscribeTransaction->proof) }}" alt="{{ $subscribeTransaction->proof }}">
                    </div>
                </div>
                <hr class="my-5">
                <form action="{{ route('admin.subscribe-transactions.update', $subscribeTransaction) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <button type="submit" class="font-bold py-2 px-4 rounded-full {{ $subscribeTransaction->is_paid == true ? 'bg-gray-300 opacity-50' : 'bg-indigo-700 hover:bg-indigo-500 text-white' }}" {{ $subscribeTransaction->is_paid == true ? 'disabled' : ''}}>
                        Approve Transaction
                    </button>
                </form>
            </div>
